...
export class LunchRecipesComponent implements OnInit {
  ...
  this.recipeService.getRecipesByCategory('almuerzo').subscribe(recipes => {
    ...
    this.filteredLunchRecipes = recipes;

    const ratingPromises = this.lunchRecipes.map(recipe => 
  ...
  sortRecipesByRating() {
    this.filteredLunchRecipes.sort((a, b) => {
  ...
  onSearch(event: any) {
    ...
    if (!searchTerm) {
      this.filteredLunchRecipes = this.lunchRecipes;
    } else {
      this.filteredLunchRecipes = this.lunchRecipes.filter(recipe =>
